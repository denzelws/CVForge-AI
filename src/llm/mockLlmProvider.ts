...
function formatCertification(certification: LlmGenerateCvDataInput["profile"]["certifications"][number]): string {
  return `${certification.name} - ${certification.issuer} (${certification.year})`;
}

function formatLanguage(language: LlmGenerateCvDataInput["profile"]["languages"][number]): string {
...
export class MockLlmProvider implements LlmProvider {
  ...
  async generateCvDataJson(input: LlmGenerateCvDataInput): Promise<GeneratedCvData> {
    ...
    const selectedExperience = input.profile.experience
      .filter((experience) =>
        input.matchReport.recommendedExperience.some((recommended) =>
          recommended.includes(experience.company)
        )
      )
      .slice(0, 2);
    ...
    return {
      ...
      experience: selectedExperience.map((experience) => ({
        ...
        bullets: pickRelevantBullets(experience.bullets, focusTerms, 4)
      })),
